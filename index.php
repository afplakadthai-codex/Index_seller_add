...
if (!function_exists('bv_member_sd_seller_owner_where')) {
    function bv_member_sd_seller_owner_where(array $orderItemCols, bool $listingExists, array $listingCols, string $orderItemAlias = 'oi', string $listingAlias = 'l', bool $allowListingOwner = true): array
    {
        $where = [];
        if (!empty($orderItemCols['seller_id'])) {
            $where[] = $orderItemAlias . '.seller_id = :seller_id';
        }
        if ($allowListingOwner && $listingExists && !empty($listingCols['seller_id'])) {
...
if (!function_exists('bvmdb_h')) {
    function bvmdb_h($value): string
    {
        return htmlspecialchars((string)$value, ENT_QUOTES, 'UTF-8');
    }
}

if (!function_exists('bvmdb_db')) {
    function bvmdb_db(): ?PDO
    {
...
if (!function_exists('bvmdb_current_user_id')) {
    function bvmdb_current_user_id(): int
    {
        $candidates = [
...
if (!function_exists('bvmdb_current_user_name')) {
    function bvmdb_current_user_name(): string
    {
        $names = [
            $_SESSION['user']['display_name'] ?? null,
            $_SESSION['user']['name'] ?? null,
            $_SESSION['auth_user']['display_name'] ?? null,
            $_SESSION['auth_user']['name'] ?? null,
            $_SESSION['member']['display_name'] ?? null,
            $_SESSION['member']['name'] ?? null,
        ];
...
if (!function_exists('bvmdb_is_logged_in')) {
    function bvmdb_is_logged_in(): bool
    {
        return bvmdb_current_user_id() > 0;
    }
}

if (!function_exists('bvmdb_table_exists')) {
    function bvmdb_table_exists(PDO $pdo, string $table): bool
    {
        static $cache = [];
        ...
        try {
            $stmt = $pdo->prepare('SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = :table LIMIT 1');
...
if (!function_exists('bvmdb_columns')) {
    function bvmdb_columns(PDO $pdo, string $table): array
    {
        static $cache = [];
        $table = strtolower(trim($table));
        if ($table === '') {
        ...
        $cache[$table] = [];
        if (!bvmdb_table_exists($pdo, $table)) {
            return [];
        }
        try {
            $stmt = $pdo->query('SHOW COLUMNS FROM `' . str_replace('`', '``', $table) . '`');
...
if (!function_exists('bvmdb_has_col')) {
    function bvmdb_has_col(PDO $pdo, string $table, string $column): bool
    {
        $cols = bvmdb_columns($pdo, $table);
        return isset($cols[$column]);
    }
}

if (!function_exists('bvmdb_build_url')) {
    function bvmdb_build_url(string $path, array $params = []): string
    {
        $path = '/' . ltrim($path, '/');
        if (!$params) {
            return $path;
...
if (!function_exists('bvmdb_money')) {
    function bvmdb_money($amount, ?string $currency = null): string
    {
...
if (!function_exists('bvmdb_order_status_badge')) {
    function bvmdb_order_status_badge(string $status): array
    {
        $value = strtolower(trim($status));
        $label = ucfirst($value !== '' ? str_replace('_', ' ', $value) : 'Unknown');
        $class = 'gray';
        if (in_array($value, ['paid', 'processing', 'confirmed', 'shipped', 'completed'], true)) {
            $class = 'green';
        } elseif (in_array($value, ['pending', 'pending_payment', 'awaiting_payment', 'new'], true)) {
            $class = 'gold';
        } elseif (in_array($value, ['cancelled', 'canceled', 'refunded', 'rejected'], true)) {
            $class = 'blue';
        }
        return [$label, $class];
    }
...
if (!function_exists('bvmdb_payment_status_badge')) {
    function bvmdb_payment_status_badge(string $status): array
    {
        return bvmdb_order_status_badge($status);
    }
}

if (!function_exists('bvmdb_refund_status_badge')) {
    function bvmdb_refund_status_badge(string $status): array
    {
        return bvmdb_order_status_badge($status);
    }
}

if (!function_exists('bvmdb_pick_column')) {
    function bvmdb_pick_column(array $cols, array $candidates): ?string
    {
        foreach ($candidates as $candidate) {
...
if (!function_exists('bvmdb_orders_summary')) {
    function bvmdb_orders_summary(PDO $pdo, int $userId): array
    {
        $summary = ['all' => 0, 'pending' => 0, 'paid_processing' => 0, 'shipped' => 0, 'completed' => 0, 'cancel_requested' => 0];
        if ($userId <= 0 || !bvmdb_table_exists($pdo, 'orders')) {
            return $summary;
        }
        $cols = bvmdb_columns($pdo, 'orders');
        $ownerCol = bvmdb_pick_column($cols, ['buyer_user_id', 'user_id', 'member_id', 'customer_id']);
        $idCol = bvmdb_pick_column($cols, ['id', 'order_id']);
        if ($ownerCol === null || $idCol === null) {
            return $summary;
        }
        $statusCol = bvmdb_pick_column($cols, ['status', 'order_status']);
        $paymentCol = bvmdb_pick_column($cols, ['payment_status', 'pay_status', 'payment_state']);
        try {
            $stmt = $pdo->prepare('SELECT * FROM `orders` WHERE `' . $ownerCol . '` = :uid');
            $stmt->execute([':uid' => $userId]);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
            $summary['all'] = count($rows);
            foreach ($rows as $row) {
                $status = strtolower(trim((string)($statusCol ? ($row[$statusCol] ?? '') : '')));
                $pay = strtolower(trim((string)($paymentCol ? ($row[$paymentCol] ?? '') : '')));
                if (in_array($status, ['pending', 'pending_payment', 'awaiting_payment', 'new'], true) || in_array($pay, ['pending', 'unpaid', 'waiting'], true)) {
                    $summary['pending']++;
                }
                if (in_array($status, ['paid', 'processing', 'confirmed'], true) || in_array($pay, ['paid', 'captured', 'completed'], true)) {
                    $summary['paid_processing']++;
                }
        ...
        if (bvmdb_table_exists($pdo, 'order_cancellations')) {
            $cancelCols = bvmdb_columns($pdo, 'order_cancellations');
            $cancelOwnerCol = bvmdb_pick_column($cancelCols, ['buyer_user_id', 'user_id', 'member_id', 'customer_id']);
            if ($cancelOwnerCol !== null) {
                try {
                    $stmt = $pdo->prepare('SELECT COUNT(*) FROM `order_cancellations` WHERE `' . $cancelOwnerCol . '` = :uid');
                    $stmt->execute([':uid' => $userId]);
                    $summary['cancel_requested'] = (int)$stmt->fetchColumn();
                } catch (Throwable $e) {
                }
            } else {
                $cancelOrderCol = bvmdb_pick_column($cancelCols, ['order_id']);
                if ($cancelOrderCol !== null) {
                    try {
                        $stmt = $pdo->prepare('SELECT COUNT(*) FROM `order_cancellations` c INNER JOIN `orders` o ON o.`' . $idCol . '` = c.`' . $cancelOrderCol . '` WHERE o.`' . $ownerCol . '` = :uid');
                        $stmt->execute([':uid' => $userId]);
                        $summary['cancel_requested'] = (int)$stmt->fetchColumn();
                    } catch (Throwable $e) {
                    }
...
if (!function_exists('bvmdb_recent_orders')) {
    function bvmdb_recent_orders(PDO $pdo, int $userId, int $limit = 5): array
    {
        if ($userId <= 0 || !bvmdb_table_exists($pdo, 'orders')) {
            return [];
        }
        ...
        $dateCol = bvmdb_pick_column($cols, ['created_at', 'order_date', 'created_on', 'updated_at']);
        $statusCol = bvmdb_pick_column($cols, ['status', 'order_status']);
        $paymentCol = bvmdb_pick_column($cols, ['payment_status', 'pay_status', 'payment_state']);
        $codeCol = bvmdb_pick_column($cols, ['order_code', 'code']);
        $totalCol = bvmdb_pick_column($cols, ['grand_total', 'total_amount', 'total', 'amount']);
        $currencyCol = bvmdb_pick_column($cols, ['currency']);
        $select = [
            '`' . $idCol . '` AS order_id',
            ($codeCol ? '`' . $codeCol . '`' : 'NULL') . ' AS order_code',
            ($dateCol ? '`' . $dateCol . '`' : 'NULL') . ' AS order_date',
            ($statusCol ? '`' . $statusCol . '`' : 'NULL') . ' AS order_status',
            ($paymentCol ? '`' . $paymentCol . '`' : 'NULL') . ' AS payment_status',
            ($totalCol ? '`' . $totalCol . '`' : '0') . ' AS total_amount',
            ($currencyCol ? '`' . $currencyCol . '`' : "'USD'") . ' AS currency',
        ];
        $sql = 'SELECT ' . implode(', ', $select) . ' FROM `orders` WHERE `' . $ownerCol . '` = :uid';
        if ($dateCol) {
            $sql .= ' ORDER BY `' . $dateCol . '` DESC';
        } else {
            $sql .= ' ORDER BY `' . $idCol . '` DESC';
        }
        $sql .= ' LIMIT ' . max(1, min(10, $limit));
        try {
...
if (!function_exists('bvmdb_recent_refunds')) {
    function bvmdb_recent_refunds(PDO $pdo, int $userId, int $limit = 5): array
    {
        if ($userId <= 0 || !bvmdb_table_exists($pdo, 'orders')) {
            return [];
        }
        $orderCols = bvmdb_columns($pdo, 'orders');
        $orderIdCol = bvmdb_pick_column($orderCols, ['id', 'order_id']);
        $orderOwnerCol = bvmdb_pick_column($orderCols, ['buyer_user_id', 'user_id', 'member_id', 'customer_id']);
        $orderCodeCol = bvmdb_pick_column($orderCols, ['order_code', 'code']);
        if ($orderIdCol === null || $orderOwnerCol === null) {
            return [];
        }
        $source = null;
        if (bvmdb_table_exists($pdo, 'order_refunds')) {
            $source = 'order_refunds';
        } elseif (bvmdb_table_exists($pdo, 'order_cancellations')) {
            $source = 'order_cancellations';
        }
        if ($source === null) {
            return [];
        }
        $srcCols = bvmdb_columns($pdo, $source);
        $srcIdCol = bvmdb_pick_column($srcCols, ['id', 'refund_id', 'cancellation_id']);
        $srcOrderCol = bvmdb_pick_column($srcCols, ['order_id']);
        if ($srcIdCol === null || $srcOrderCol === null) {
            return [];
        }
        $srcCodeCol = bvmdb_pick_column($srcCols, ['refund_code', 'request_code', 'cancel_code', 'code']);
        $srcStatusCol = bvmdb_pick_column($srcCols, ['status', 'refund_status', 'cancel_status']);
        $srcAmountCol = bvmdb_pick_column($srcCols, ['requested_refund_amount', 'requested_amount', 'refund_amount', 'amount']);
        $srcDateCol = bvmdb_pick_column($srcCols, ['updated_at', 'requested_at', 'created_at']);
        $srcCurrencyCol = bvmdb_pick_column($srcCols, ['currency']);
        $select = [
            'r.`' . $srcIdCol . '` AS request_id',
            ($srcCodeCol ? 'r.`' . $srcCodeCol . '`' : 'NULL') . ' AS request_code',
            'o.`' . $orderIdCol . '` AS order_id',
            ($orderCodeCol ? 'o.`' . $orderCodeCol . '`' : 'NULL') . ' AS order_code',
            ($srcStatusCol ? 'r.`' . $srcStatusCol . '`' : "'pending'") . ' AS request_status',
            ($srcAmountCol ? 'r.`' . $srcAmountCol . '`' : '0') . ' AS request_amount',
            ($srcDateCol ? 'r.`' . $srcDateCol . '`' : 'NULL') . ' AS updated_at',
            ($srcCurrencyCol ? 'r.`' . $srcCurrencyCol . '`' : "'USD'") . ' AS currency',
        ];
        $sql = 'SELECT ' . implode(', ', $select) . ' FROM `' . $source . '` r INNER JOIN `orders` o ON o.`' . $orderIdCol . '` = r.`' . $srcOrderCol . '` WHERE o.`' . $orderOwnerCol . '` = :uid';
        if ($srcDateCol) {
            $sql .= ' ORDER BY r.`' . $srcDateCol . '` DESC';
        } else {
            $sql .= ' ORDER BY r.`' . $srcIdCol . '` DESC';
        }
        $sql .= ' LIMIT ' . max(1, min(10, $limit));
        try {
            $stmt = $pdo->prepare($sql);
...
$sellerRefundMonthly = [];
$rankingSnapshot = [];
$rankingSnapshotAvailable = false;

$userId = (int)($user['id'] ?? 0);
$memberDashboardSummary = ['all' => 0, 'pending' => 0, 'paid_processing' => 0, 'shipped' => 0, 'completed' => 0, 'cancel_requested' => 0];
$memberRecentOrders = [];
$memberRecentRefunds = [];
$buyerProfile = null;


$memberDashboardUserId = $userId > 0 ? $userId : bvmdb_current_user_id();
$memberDashboardPdo = bvmdb_db();
if ($memberDashboardPdo instanceof PDO && $memberDashboardUserId > 0 && bvmdb_is_logged_in()) {
    $memberDashboardSummary = bvmdb_orders_summary($memberDashboardPdo, $memberDashboardUserId);
    $memberRecentOrders = bvmdb_recent_orders($memberDashboardPdo, $memberDashboardUserId, 10);
    $memberRecentRefunds = bvmdb_recent_refunds($memberDashboardPdo, $memberDashboardUserId, 10);
}
...
if ($isSellerApproved && $userId > 0) {
    ...
    // Cast to int once — used inline in SQL to avoid duplicate named-placeholder issues
    ...
    // --- Resolve ownership columns ONCE so every KPI block below can use them safely ---
    $_kpi_orderItemCols  = bv_member_sd_table_exists($pdo, 'order_items') ? bv_member_sd_columns($pdo, 'order_items') : [];
    $_kpi_listingExists  = bv_member_sd_table_exists($pdo, 'listings');
    $_kpi_listingCols    = $_kpi_listingExists ? bv_member_sd_columns($pdo, 'listings') : [];
    $_kpi_hasListingJoin = ($_kpi_listingExists && !empty($_kpi_orderItemCols['listing_id']));
    // Build ownership WHERE using inlined integer — no named placeholder, no HY093
    $_kpi_ownerWhereParts = [];
    if (!empty($_kpi_orderItemCols['seller_id'])) {
        $_kpi_ownerWhereParts[] = 'oi.seller_id = ' . $sellerIdInt;
    }
    if ($_kpi_hasListingJoin && !empty($_kpi_listingCols['seller_id'])) {
        $_kpi_ownerWhereParts[] = 'l.seller_id = ' . $sellerIdInt;
    }
    $_kpi_ownerWhere     = $_kpi_ownerWhereParts; // array of SQL fragments, no placeholders
    $_kpi_ownerMode      = !empty($_kpi_orderItemCols['seller_id']) ? 'oi.seller_id' : (!empty($_kpi_listingCols['seller_id']) ? 'l.seller_id' : 'none');
    $_kpi_last_error     = '';
    $_kpi_matched_items  = 0;
    $_kpi_agg_ok         = false;
    $_kpi_recent_ok      = false;
    $_kpi_refunds_ok     = false;
    $_kpi_monthly_ok     = false;

    // --- DEBUG: count matched seller order_items ---
    if ($_kpi_ownerWhere && bv_member_sd_table_exists($pdo, 'order_items')) {
        try {
            $_kpi_lj_debug = $_kpi_hasListingJoin ? 'LEFT JOIN listings l ON l.id = oi.listing_id' : '';
            $_stmt_dbg = $pdo->query(
                'SELECT COUNT(*) FROM order_items oi ' . $_kpi_lj_debug .
                ' WHERE (' . implode(' OR ', $_kpi_ownerWhere) . ')'
            );
            $_kpi_matched_items = $_stmt_dbg ? (int)$_stmt_dbg->fetchColumn() : 0;
        } catch (Throwable $_kpi_ex) {
            $_kpi_last_error = 'debug count: ' . $_kpi_ex->getMessage();
        }
    }

    // --- BLOCK A: Aggregate KPI stats ---
    // Uses inlined integer values and bindValue for dates to avoid duplicate-placeholder HY093
    try {
        if (bv_member_sd_table_exists($pdo, 'orders') && bv_member_sd_table_exists($pdo, 'order_items') && $_kpi_ownerWhere) {
            ...
            // Inline month dates as quoted literals to avoid repeated named placeholders
            $monthStartQ     = $pdo->quote($monthStart);
            $nextMonthStartQ = $pdo->quote($nextMonthStart);

            $sql = "
                SELECT
                    COUNT(DISTINCT o.id) AS total_seller_orders,
                    COUNT(DISTINCT CASE WHEN ({$createdExpr} >= {$monthStartQ} AND {$createdExpr} < {$nextMonthStartQ}) THEN o.id END) AS this_month_orders,
                    SUM(CASE WHEN {$createdExpr} >= {$monthStartQ} AND {$createdExpr} < {$nextMonthStartQ}
                             AND ({$orderStatusExpr} IN ('paid','confirmed','processing','packing','shipped','completed','refunded')
                                  OR {$paymentStatusExpr} IN ('paid','captured','completed','success'))
                        THEN COALESCE({$lineTotalExpr}, 0) ELSE 0 END) AS this_month_sales,
                    COUNT(DISTINCT CASE WHEN {$orderStatusExpr} IN ('pending','pending_payment','reserved') THEN o.id END) AS new_orders,
                    COUNT(DISTINCT CASE WHEN {$orderStatusExpr} IN ('paid','confirmed','processing','packing')
                                           OR {$paymentStatusExpr} IN ('paid','captured','completed','success') THEN o.id END) AS paid_orders,
                ...
                WHERE (" . implode(' OR ', $_kpi_ownerWhere) . ")
            ";
            // No bound parameters needed — seller_id inlined as int, dates inlined via quote()
            $stmt = $pdo->query($sql);
        ...
        $_kpi_last_error = 'agg: ' . $e->getMessage();
        error_log('member index seller aggregate stats load failed: ' . $e->getMessage());
    }

    // --- BLOCK B: Recent seller orders ---
    try {
        if (bv_member_sd_table_exists($pdo, 'orders') && bv_member_sd_table_exists($pdo, 'order_items') && $_kpi_ownerWhere) {
            $orderCols            = bv_member_sd_columns($pdo, 'orders');
            $orderItemCols        = $_kpi_orderItemCols;
            $listingColsForOrders = $_kpi_listingCols;
            ...
            $listingTitleExpr = !empty($listingColsForOrders['title']) ? 'l.title'
                : (!empty($orderItemCols['title_snapshot']) ? 'oi.title_snapshot' : "CONCAT('Listing #', oi.listing_id)");

            $buyerJoinCol = 'o.user_id';
            if (!empty($orderCols['buyer_user_id'])) $buyerJoinCol = 'o.buyer_user_id';
            elseif (!empty($orderCols['member_id'])) $buyerJoinCol = 'o.member_id';

            $listingJoin = $_kpi_hasListingJoin ? 'LEFT JOIN listings l ON l.id = oi.listing_id' : '';

            // No params needed — ownership WHERE already inlined
            $sql = "
                SELECT
                    o.id AS order_id,
                ...
                INNER JOIN order_items oi ON oi.order_id = o.id
                {$listingJoin}
                LEFT JOIN users u ON u.id = {$buyerJoinCol}
                WHERE (" . implode(' OR ', $_kpi_ownerWhere) . ")
                GROUP BY o.id
            ...
            ";
            $stmt = $pdo->query($sql);
            $sellerRecentOrders = $stmt ? ($stmt->fetchAll(PDO::FETCH_ASSOC) ?: []) : [];
            $_kpi_recent_ok = true;
        }
    } catch (Throwable $e) {
        $_kpi_last_error = 'recent_orders: ' . $e->getMessage();
        error_log('member index seller recent orders load failed: ' . $e->getMessage());
    }

    // --- BLOCK C: Recent seller refunds ---
    try {
        if (bv_member_sd_table_exists($pdo, 'order_refunds') && bv_member_sd_table_exists($pdo, 'order_refund_items') && bv_member_sd_table_exists($pdo, 'order_items') && $_kpi_ownerWhere) {
            $refundCols           = bv_member_sd_columns($pdo, 'order_refunds');
            $refundItemCols       = bv_member_sd_columns($pdo, 'order_refund_items');
            $orderItemCols        = $_kpi_orderItemCols;
            $orderCols            = bv_member_sd_columns($pdo, 'orders');
            $listingColsForRefunds = $_kpi_listingCols;
            ...
            // Item-level seller-scoped amounts — prefer ri.* columns, never use header r.* totals directly
            $riRequestedExpr = !empty($refundItemCols['requested_refund_amount']) ? 'ri.requested_refund_amount'
                : (!empty($refundItemCols['refund_line_amount']) ? 'ri.refund_line_amount'
                : (!empty($refundItemCols['line_total_snapshot']) ? 'ri.line_total_snapshot' : '0'));
            $riApprovedExpr  = !empty($refundItemCols['approved_refund_amount'])  ? 'ri.approved_refund_amount'
                : (!empty($refundItemCols['actual_refund_after_fee']) ? 'ri.actual_refund_after_fee'
                : (!empty($refundItemCols['actual_refunded_amount']) ? 'ri.actual_refunded_amount'
                : $riRequestedExpr));

            $refundBuyerCandidates = [];
            if (!empty($orderCols['buyer_name']))    $refundBuyerCandidates[] = "NULLIF(TRIM(o.buyer_name), '')";
            if (!empty($orderCols['customer_name'])) $refundBuyerCandidates[] = "NULLIF(TRIM(o.customer_name), '')";
            if (!empty($orderCols['ship_name']))     $refundBuyerCandidates[] = "NULLIF(TRIM(o.ship_name), '')";
            if (!empty($refundUserCols['first_name']) && !empty($refundUserCols['last_name'])) {
                $refundBuyerCandidates[] = "NULLIF(TRIM(CONCAT(COALESCE(u.first_name, ''), ' ', COALESCE(u.last_name, ''))), '')";
            }
            if (!empty($refundUserCols['email']))   $refundBuyerCandidates[] = "NULLIF(TRIM(u.email), '')";
            if (!empty($orderCols['buyer_email']))  $refundBuyerCandidates[] = "NULLIF(TRIM(o.buyer_email), '')";
            $refundBuyerNameExpr = $refundBuyerCandidates ? 'COALESCE(' . implode(', ', $refundBuyerCandidates) . ", 'Customer')" : "'Customer'";

            $refundBuyerJoinCol = 'o.user_id';
            if (!empty($orderCols['buyer_user_id'])) $refundBuyerJoinCol = 'o.buyer_user_id';
            elseif (!empty($orderCols['member_id'])) $refundBuyerJoinCol = 'o.member_id';
            ...
            // Build refund ownership WHERE inlined — same pattern as main ownership but for refund context
            $refundOwnerWhere = [];
            if (!empty($orderItemCols['seller_id'])) {
                $refundOwnerWhere[] = 'oi.seller_id = ' . $sellerIdInt;
            }
            if ($hasRefundListingJoin && !empty($listingColsForRefunds['seller_id'])) {
            ...
            $sql = "
                SELECT
                    ...
                    SUM(COALESCE({$riRequestedExpr}, 0)) AS requested_refund_amount,
                    SUM(COALESCE({$riApprovedExpr}, 0)) AS approved_refund_amount,
                ...
                LEFT JOIN users u ON u.id = {$refundBuyerJoinCol}
            ...
            ";
            $stmt = $pdo->query($sql);
            $sellerRecentRefunds = $stmt ? ($stmt->fetchAll(PDO::FETCH_ASSOC) ?: []) : [];
            ...
            $_kpi_refunds_ok = true;
        }
    } catch (Throwable $e) {
        $_kpi_last_error = 'refunds: ' . $e->getMessage();
        error_log('member index seller refunds load failed: ' . $e->getMessage());
    }

    // --- BLOCK D: Seller offers ---
    try {
        if (bv_member_sd_table_exists($pdo, 'listing_offers')) {
            $offerCols           = bv_member_sd_columns($pdo, 'listing_offers');
            $listingColsForOffers = $_kpi_listingCols;
            $userCols            = bv_member_sd_table_exists($pdo, 'users') ? bv_member_sd_columns($pdo, 'users') : [];

            // Build offer ownership WHERE inlined
            $offerOwnerWhere = [];
            if (!empty($offerCols['seller_user_id'])) {
                $offerOwnerWhere[] = 'o.seller_user_id = ' . $sellerIdInt;
            }
            if ($_kpi_listingExists && !empty($offerCols['listing_id']) && !empty($listingColsForOffers['seller_id'])) {
                $offerOwnerWhere[] = 'l.seller_id = ' . $sellerIdInt;
            }
            if (!$offerOwnerWhere) {
            ...
            $listingJoin = ($_kpi_listingExists && !empty($offerCols['listing_id'])) ? 'LEFT JOIN listings l ON l.id = o.listing_id' : '';
            ...
            ";
            $stmt = $pdo->query($sql);
            $sellerRecentOfferThreads = $stmt ? ($stmt->fetchAll(PDO::FETCH_ASSOC) ?: []) : [];
    ...
    try {
        if (bv_member_sd_table_exists($pdo, 'orders') && bv_member_sd_table_exists($pdo, 'order_items') && $_kpi_ownerWhere) {
            ...
            ";
            $stmt = $pdo->query($sql);
            $sellerSalesMonthly = $stmt ? ($stmt->fetchAll(PDO::FETCH_ASSOC) ?: []) : [];
            ...
            $_kpi_monthly_ok = true;
        }
    } catch (Throwable $e) {
        $_kpi_last_error = 'monthly_sales: ' . $e->getMessage();
        error_log('member index seller sales monthly load failed: ' . $e->getMessage());
    }

    // --- BLOCK F: Monthly refunds ---
    try {
        if (bv_member_sd_table_exists($pdo, 'order_refunds') && bv_member_sd_table_exists($pdo, 'order_refund_items') && bv_member_sd_table_exists($pdo, 'order_items') && $_kpi_ownerWhere) {
            $refundCols     = bv_member_sd_columns($pdo, 'order_refunds');
            $refundItemCols = bv_member_sd_columns($pdo, 'order_refund_items');
            ...
            // Item-level seller-scoped refunded amount — prefer ri.* columns
            $riMonthApproved = !empty($refundItemCols['approved_refund_amount'])  ? 'ri.approved_refund_amount'
                : (!empty($refundItemCols['actual_refund_after_fee']) ? 'ri.actual_refund_after_fee'
                : (!empty($refundItemCols['actual_refunded_amount']) ? 'ri.actual_refunded_amount'
                : (!empty($refundItemCols['requested_refund_amount']) ? 'ri.requested_refund_amount'
                : (!empty($refundItemCols['line_total_snapshot']) ? 'ri.line_total_snapshot' : '0'))));

            $hasRefundListingJoin = ($_kpi_listingExists && (!empty($refundItemCols['listing_id']) || !empty($orderItemCols['listing_id'])));
            $listingJoin = $hasRefundListingJoin ? 'LEFT JOIN listings l ON l.id = COALESCE(ri.listing_id, oi.listing_id)' : '';

            // Build refund monthly ownership WHERE inlined
            $refundMonthOwnerWhere = [];
            if (!empty($orderItemCols['seller_id'])) {
                $refundMonthOwnerWhere[] = 'oi.seller_id = ' . $sellerIdInt;
            }
            if ($hasRefundListingJoin && !empty($_kpi_listingCols['seller_id'])) {
                $refundMonthOwnerWhere[] = 'l.seller_id = ' . $sellerIdInt;
            }
            if (!$refundMonthOwnerWhere) {
            ...
            $sql = "
                SELECT
                    ...
                    SUM(CASE WHEN LOWER(COALESCE({$refundStatusExpr}, '')) IN ('approved','processing','refunded','completed')
                        THEN COALESCE({$riMonthApproved}, 0) ELSE 0 END) AS refunded_amount,
                    MAX(COALESCE({$refundCurrencyExpr}, 'USD')) AS currency
                FROM order_refunds r
                INNER JOIN order_refund_items ri ON ri.refund_id = r.id
                LEFT JOIN order_items oi ON oi.id = ri.order_item_id
                {$listingJoin}
                WHERE (" . implode(' OR ', $refundMonthOwnerWhere) . ")
                GROUP BY DATE_FORMAT({$refundMonthExpr}, '%Y-%m')
                ORDER BY sales_month DESC
                LIMIT 6
            ";
            $stmt = $pdo->query($sql);
            $sellerRefundMonthly = $stmt ? ($stmt->fetchAll(PDO::FETCH_ASSOC) ?: []) : [];
    ...
    // No fatal errors if tables are missing.
    if (!function_exists('bv_member_sd_ranking_snapshot')) {
        function bv_member_sd_ranking_snapshot(PDO $pdo, int $sellerId): array
        {
            $empty = [
                'total_ranked'   => 0,
                'avg_score'      => null,
            ...
            if (!bv_member_sd_table_exists($pdo, 'listing_ranking_scores')
                || !bv_member_sd_table_exists($pdo, 'listings')) {
                return $empty;
            }
            try {
                $stmt = $pdo->prepare('
                    SELECT
                        ...
                        ROUND(MAX(r.final_score), 2)  AS best_score,
                        ROUND(MIN(r.final_score), 2)  AS lowest_score,
                        MAX(r.calculated_at)           AS last_calculated
                    FROM listing_ranking_scores r
                ...
                $row = $stmt->fetch(PDO::FETCH_ASSOC) ?: [];
                return [
                    'total_ranked'    => (int)($row['total_ranked']    ?? 0),
                    'avg_score'       => isset($row['avg_score'])    && $row['avg_score']    !== null ? (float)$row['avg_score']    : null,
                    'best_score'      => isset($row['best_score'])    && $row['best_score']   !== null ? (float)$row['best_score']   : null,
                    'lowest_score'    => isset($row['lowest_score'])  && $row['lowest_score'] !== null ? (float)$row['lowest_score'] : null,
                    'last_calculated' => $row['last_calculated'] ?? null,
                ];
            } catch (Throwable $e) {
                error_log('member index ranking snapshot load failed: ' . $e->getMessage());
                return $empty;
            }
        }
    }
    ...
    try {
        $rankingSnapshot = bv_member_sd_ranking_snapshot($pdo, $sellerIdInt);
        $rankingSnapshotAvailable = ($rankingSnapshot['total_ranked'] > 0);
    } catch (Throwable $e) {
        error_log('member index ranking snapshot block failed: ' . $e->getMessage());
        $rankingSnapshot = ['total_ranked' => 0, 'avg_score' => null, 'best_score' => null, 'lowest_score' => null, 'last_calculated' => null];
        $rankingSnapshotAvailable = false;
    }
}
  ...
  <a class="btn-outline" href="/member/order_view.php">Open</a>
    ...
    <section class="panel">
      <div class="panel-body">
        <div class="hero" style="margin-bottom:14px;">
          <div>
            <h1 style="font-size:24px;margin-bottom:6px;">My Account Activity</h1>
            <p>Track your latest order and refund activity directly from your member dashboard.</p>
          </div>
        </div>

        <div class="stat-grid" style="margin-bottom:14px;">
          <div class="stat-card"><strong><?= (int)$memberDashboardSummary['all'] ?></strong><span>All Orders</span></div>
          <div class="stat-card"><strong><?= (int)$memberDashboardSummary['pending'] ?></strong><span>Pending / Pending Payment</span></div>
          <div class="stat-card"><strong><?= (int)$memberDashboardSummary['paid_processing'] ?></strong><span>Paid / Processing</span></div>
          <div class="stat-card"><strong><?= (int)$memberDashboardSummary['shipped'] ?></strong><span>Shipped</span></div>
          <div class="stat-card"><strong><?= (int)$memberDashboardSummary['completed'] ?></strong><span>Completed</span></div>
          <div class="stat-card"><strong><?= (int)$memberDashboardSummary['cancel_requested'] ?></strong><span>Cancel Requested</span></div>
        </div>

        <div class="mini-card" style="margin-bottom:14px;">
          <h3>Recent Orders</h3>
          <?php if (!$memberRecentOrders): ?>
          ...
          <?php else: ?>
            <div class="table-wrap">
              <table class="data-table">
                ...
                <tbody>
                  <?php foreach ($memberRecentOrders as $row): ?>
                    <?php
                    ...
                    $orderDateTs = strtotime($orderDateRaw);
                    $orderDate = $orderDateTs ? date('Y-m-d H:i', $orderDateTs) : '-';
                    [$orderLabel, $orderClass] = bvmdb_order_status_badge((string)($row['order_status'] ?? ''));
                    [$payLabel, $payClass] = bvmdb_payment_status_badge((string)($row['payment_status'] ?? ''));
                    $orderActionUrl = bvmdb_build_url('/member/order_detail.php', ['id' => $orderId]);
                    ?>
                    <tr>
                      <td><?= bvmdb_h($orderCode) ?></td>
                      <td><?= bvmdb_h($orderDate) ?></td>
                      <td><span class="status-pill <?= bvmdb_h($orderClass) ?>"><?= bvmdb_h($orderLabel) ?></span></td>
                      <td><span class="status-pill <?= bvmdb_h($payClass) ?>"><?= bvmdb_h($payLabel) ?></span></td>
                      <td><?= bvmdb_h(bvmdb_money($row['total_amount'] ?? 0, (string)($row['currency'] ?? 'USD'))) ?></td>
                      <td><a class="btn-soft" href="<?= bvmdb_h($orderActionUrl) ?>">View</a></td>
                    </tr>
                  <?php endforeach; ?>
                </tbody>
              </table>
        ...
        <div class="mini-card">
          ...
          <?php if (!$memberRecentRefunds): ?>
            <div class="empty">No refund requests yet.</div>
          <?php else: ?>
            <div class="table-wrap">
              <table class="data-table">
                <thead>
                ...
                <tbody>
                  <?php foreach ($memberRecentRefunds as $row): ?>
                    <?php
                    ...
                    [$refundLabel, $refundClass] = bvmdb_refund_status_badge((string)($row['request_status'] ?? ''));
                    $updatedRaw = (string)($row['updated_at'] ?? '');
                    $updatedTs = strtotime($updatedRaw);
                    $updatedAt = $updatedTs ? date('Y-m-d H:i', $updatedTs) : '-';
                    $refundActionUrl = bvmdb_build_url('/member/order_detail.php', ['id' => $orderId]);
                    ?>
                    <tr>
                      <td><?= bvmdb_h($requestCode) ?></td>
                      <td><?= bvmdb_h($orderCode) ?></td>
                      <td><span class="status-pill <?= bvmdb_h($refundClass) ?>"><?= bvmdb_h($refundLabel) ?></span></td>
                      <td><?= bvmdb_h(bvmdb_money($row['request_amount'] ?? 0, (string)($row['currency'] ?? 'USD'))) ?></td>
                      <td><?= bvmdb_h($updatedAt) ?></td>
                      <td><a class="btn-soft" href="<?= bvmdb_h($refundActionUrl) ?>">View</a></td>
                    </tr>
                  <?php endforeach; ?>
                </tbody>
              </table>
    ...
    <?php if ($isSellerApproved): ?>
      ...
      <section class="panel">
        <div class="panel-body">
          <div class="hero" style="margin-bottom:14px;">
            <div>
              <h1 style="font-size:24px;margin-bottom:6px;">Listing Ranking Snapshot</h1>
              <p>A quick summary of how your listings are currently ranked across the marketplace.</p>
            </div>
            <div class="actions">
              <a class="btn-outline" href="/seller/ranking.php">View Full Ranking Dashboard</a>
            </div>
          </div>

          <?php if (!$rankingSnapshotAvailable): ?>
            <div class="empty">Ranking data is not available yet. Scores are generated automatically after listings are recalculated.</div>
          <?php else: ?>
            <?php
            $rAvg = $rankingSnapshot['avg_score'];
            if ($rAvg !== null && $rAvg >= 80)     { $rankLabel = 'Excellent'; $rankPill = 'green'; }
            elseif ($rAvg !== null && $rAvg >= 70)  { $rankLabel = 'Good';      $rankPill = 'blue';  }
            elseif ($rAvg !== null && $rAvg >= 50)  { $rankLabel = 'Average';   $rankPill = 'gold';  }
            else                                    { $rankLabel = 'Needs Attention'; $rankPill = 'gray'; }
            ?>
            <div class="stat-grid" style="grid-template-columns:repeat(5,minmax(0,1fr));margin-bottom:16px;">
              <div class="stat-card">
                <strong><?= $rAvg !== null ? bv_member_e(number_format($rAvg, 1)) : '—' ?></strong>
                <span>Avg. Score</span>
              </div>
              <div class="stat-card">
                <strong><?= $rankingSnapshot['best_score'] !== null ? bv_member_e(number_format((float)$rankingSnapshot['best_score'], 1)) : '—' ?></strong>
                <span>Best Score</span>
              </div>
              <div class="stat-card">
                <strong><?= $rankingSnapshot['lowest_score'] !== null ? bv_member_e(number_format((float)$rankingSnapshot['lowest_score'], 1)) : '—' ?></strong>
                <span>Lowest Score</span>
              </div>
              <div class="stat-card">
                <strong><?= (int)$rankingSnapshot['total_ranked'] ?></strong>
                <span>Ranked Listings</span>
              </div>
              <div class="stat-card">
                <strong><span class="status-pill <?= $rankPill ?>"><?= bv_member_e($rankLabel) ?></span></strong>
                <span>Overall Health</span>
              </div>
            </div>
            <?php if (!empty($rankingSnapshot['last_calculated'])): ?>
              <div class="muted" style="font-size:13px;">Last calculated: <?= bv_member_e(date('Y-m-d H:i', strtotime((string)$rankingSnapshot['last_calculated']) ?: time())) ?></div>
            <?php endif; ?>
          <?php endif; ?>
        </div>
      </section>
